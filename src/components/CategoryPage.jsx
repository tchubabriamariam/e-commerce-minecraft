...
function CategoryPage() {
  ...
  const convertedPrice = (products.price * rates[currency]).toFixed(2);
  ...
  return (
    <div className="categoryPage">
      <h1 className="categoryTitle">{capitalizedCategory}</h1>
      <div className="products">
        {filtered.map((product) => (
          <div
            className={`productCard ${product.outofStock ? "outofstock" : ""}`}
            key={product.id}
